...
<div class="flex">
    ...
    <div id="customer_dropdown" class="hidden absolute z-10 w-full mt-1 bg-white border rounded shadow-lg max-h-60 overflow-y-auto"></div>
    ...
    <div id="new_customer_fields" class="hidden mt-2">
        <div class="grid md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" id="new_customer_phone" class="border rounded px-3 py-2 w-full" placeholder="Nomor telepon">
            </div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // --- CUSTOMER SEARCH (sama persis seperti implementasimu, hanya dibungkus DOMContentLoaded) ---
    ...
    if (customerSearch) {
        customerSearch.addEventListener('input', function() {
            ...
            if (filteredCustomers.length > 0) {
                ...
                customerDropdown.innerHTML = `
                    <div class="p-2 hover:bg-gray-100 cursor-pointer" id="create_new_customer">
                        <span class="text-blue-600">+ Buat customer baru: "${searchTerm}"</span>
                    </div>
                `;
                customerDropdown.classList.remove('hidden');
            }
        ...
        customerDropdown.addEventListener('click', function(e) {
            ...
            if (opt) {
                selectCustomer(opt.dataset.id, opt.dataset.name, opt.dataset.phone);
            } else {
                const createBtn = e.target.closest('#create_new_customer');
                if (createBtn) {
                    const searchTerm = customerSearch.value;
                    document.getElementById('new_customer_name').value = searchTerm;
