        ...
        <div class="flex-1 lg:w-5/6">
            ...
            <div class="p-4 lg:p-8">
                <div class="bg-white p-6 rounded-xl shadow-lg mb-6">
                    ...
                    @endif
                    <form action="{{ route('kepala-toko.sales.update', $salesOrder) }}" method="POST" id="soForm"
                        ...
                        <div class="grid md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <div class="flex items-center space-x-4">
                                    <label><input type="radio" name="order_type" value="jahit_sendiri" {{ $salesOrder->order_type == 'jahit_sendiri' ? 'checked' : '' }}
                                            class="mr-2">Jahit Sendiri</label>
                                    <label><input type="radio" name="order_type" value="beli_jadi" {{ $salesOrder->order_type == 'beli_jadi' ? 'checked' : '' }}
                                            class="mr-2">Langsung Beli Jadi</label>
                                </div>
                                @error('order_type')
                            ...
                            <div class="relative">
                                ...
                                <input type="hidden" name="customer_phone" id="customer_phone"
                                    value="{{ old('customer_phone', $salesOrder->customer ? $salesOrder->customer->phone : '') }}">
                                ...
                                <div id="selected_customer"
                                    class="mt-2 p-2 bg-blue-50 rounded {{ $salesOrder->customer ? '' : 'hidden' }}">
                                    <span id="customer_display_name"
                                        class="font-medium">{{ $salesOrder->customer ? $salesOrder->customer->name : '' }}</span>
                                    <span id="customer_display_phone"
                                        class="text-sm text-gray-600 ml-2">{{ $salesOrder->customer && $salesOrder->customer->phone ? '(' . $salesOrder->customer->phone . ')' : '' }}</span>
                            ...
                            <div class="grid md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" name="payment_amount" id="payment_amount"
                                        class="border rounded px-3 py-2 w-full focus:ring focus:ring-blue-300"
                                        step="0.01" min="0"
                                        value="{{ old('payment_amount', $salesOrder->paid_total) }}">
                                    ...
                                    <div class="grid md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="number" name="cash_amount" id="cash_amount"
                                                class="border rounded px-3 py-2 w-full focus:ring focus:ring-blue-300"
                                                step="0.01" min="0"
                                                value="{{ old('cash_amount', $salesOrder->payments->first()->cash_amount ?? 0) }}">
                                            @error('cash_amount')
                                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                            @enderror
                                        </div>
                                        <div>
                                            ...
                                            <input type="number" name="transfer_amount" id="transfer_amount"
                                                class="border rounded px-3 py-2 w-full focus:ring focus:ring-blue-300"
                                                step="0.01" min="0"
                                                value="{{ old('transfer_amount', $salesOrder->payments->first()->transfer_amount ?? 0) }}">
                                    ...
                                    <div class="item-row grid md:grid-cols-5 gap-4">
                                        <div>
                                            ...
                                            <select name="items[{{ $index }}][product_id]"
                                                class="product-select border rounded px-3 py-2 w-full focus:ring focus:ring-blue-300">
                                                <option value="">Pilih Produk</option>
                                                @foreach($products as $product)
                                                    <option value="{{ $product->id }}" data-name="{{ $product->name }}"
                                                        data-sku="{{ $product->sku }}" data-price="{{ $product->price }}" {{ $item->product_id == $product->id ? 'selected' : '' }}>
                                                        {{ $product->name }}</option>
                                                @endforeach
                                            </select>
                                            <input type="hidden" name="items[{{ $index }}][product_name]"
                                                class="product-name" value="{{ $item->product_name }}">
                                        ...
                                        <div>
                                            ...
                                            <input type="number" name="items[{{ $index }}][discount]"
                                                ...
                                                min="0" step="0.01" value="{{ $item->discount ?? 0 }}">
                                            @error("items.$index.discount")
                                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                            @enderror
                                            @if($index > 0)
                                                <button type="button"
                                                    class="remove-item text-red-600 hover:text-red-800 mt-2"><i
