<div class="flex justify-between items-center p-4 bg-white shadow">
    ...
    <div class="flex items-center space-x-4">
        <!-- Notifikasi -->
        <a href="{{ route('inventaris.notification.index') }}" id="notification-bell" class="relative text-gray-500 text-xl cursor-pointer">
        ...
        <div class="flex items-center">
            ...
            <a href="{{ route('inventaris.profile.index') }}" class="flex items-center hover:opacity-80 transition-opacity">
                @php
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        function updateNotificationCount() {
            fetch('/inventaris/notifications/unread-count')
        ...
        document.getElementById('notification-bell').addEventListener('click', function (e) {
            e.preventDefault();
            fetch('/inventaris/notifications/mark-as-read', { 
                method: 'POST', 
                headers: {
            ...
            .then(() => {
                updateNotificationCount();
                window.location.href = '{{ route('inventaris.notification.index') }}';
            });
        });
    });
</script>